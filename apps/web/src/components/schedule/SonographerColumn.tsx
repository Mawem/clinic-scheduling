...
export function SonographerColumn({
  ...
  return (
    <div className="min-w-[150px]">
      <div className="mb-2 flex items-center gap-2 rounded-lg bg-white px-3 py-2 shadow-sm ring-1 ring-slate-900/5">
        <span aria-hidden="true" className={clsx("size-2.5 rounded-full", colors.dot)} />
        <div>
          <p className="text-sm font-semibold text-slate-900">{sonographer.name}</p>
          <p className="text-xs text-slate-500">{sonographer.credentials}</p>
        </div>
      </div>
      ...
      <div
        ...
        className="relative rounded-lg bg-white shadow-sm ring-1 ring-slate-900/5"
